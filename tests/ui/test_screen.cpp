...
static ImGuiID get_popup_item_id(const char *popup_id_substr,
  ...
  for (int i = 0; i < g.Windows.Size; i++) {
    ...
    if (strstr(g.Windows[i]->Name, "##Popup_") ||
        strstr(g.Windows[i]->Name, popup_id_substr)) {
...
TEST_F(PresetTest,
       LooperDisplay_OverdubButton_Click_CallsRequestOverdubToggle) {
  ScopedImGuiContext imgui;
  ImGui::Begin("TestWindow");
  ImDrawList *dl = ImGui::GetWindowDrawList();
  ImVec2 p0 = ImVec2(100, 100);
...
TEST_F(PresetTest, LooperDisplay_LevelSlider_Change_ClampsAndPushesToEngine) {
  ...
  // Center is at X = 210.
  // display_y of slider is around p0.y + 147 = 247.
  // Vertical center is around Y = 255.

  // First layout the slider
  ScreenComponent::render(dl, p0, 220.0f, 1.0f, props);
  io.MousePos = ImVec2(210.0f, 255.0f);
  advance_frame();
  ...
  // Now, prepare to simulate a press on the left side of the slider
  io.MousePos = ImVec2(150.0f, 255.0f);
  io.MouseDown[0] = true;
  advance_frame();

  // Click frame: slider becomes active
  ScreenComponent::render(dl, p0, 220.0f, 1.0f, props);

  // Now, drag it to the right side of the slider
  io.MousePos = ImVec2(250.0f, 255.0f);
  advance_frame();

  // Drag frame: slider value is updated
  ScreenComponent::render(dl, p0, 220.0f, 1.0f, props);
  ...
  // Release frame: slider is deactivated, triggering commit callback
  ScreenComponent::render(dl, p0, 220.0f, 1.0f, props);
  advance_frame();
...
TEST_F(PresetTest, MBKnob_VerticalDrag_UpdatesValueAndPushesToEngine) {
  ...
  ScreenComponent::render(dl, p0, 220.0f, 1.0f, props);
  advance_frame();
  ScreenComponent::render(dl, p0, 220.0f, 1.0f,
                          props); // IsItemHovered() is true
  advance_frame();
...
TEST_F(PresetTest, MultiBandCompressor_MBKnobPopupAndInteractions) {
  ...
  auto comp = std::make_shared<MultiBandCompressor>();
  comp->params_[2].value =
      -10.0f; // threshold default is typically -20.0f, let's change it
  ...
  char popup_id[128];
  std::snprintf(popup_id, sizeof(popup_id), "##knob_%s_%d_%d_%s", comp->name(),
                4, 2, "thresh");
  ...
  // Slider horizontal center is at pop_pos.x + 70.0f, vertical center is
  // pop_pos.y + 45.0f
  ...
  ImGuiID remove_bp_id =
      get_popup_item_id(popup_id, "Remove MIDI Bypass Mapping");
  ...
  // ==========================================
  midi_manager.start_learn(MidiTargetType::EffectParam, comp->name(),
                           comp->params_[2].name);
  ScreenComponent::render(dl, p0, 220.0f, 1.0f, props);
  advance_frame();
  midi_manager.cancel_learn();
...
TEST_F(PresetTest, MultiBandCompressor_XoverOverlapPrevention) {
  ...
  // ==========================================
  comp->params_[0].value = 450.0f;
  comp->params_[1].value =
      100.0f; // low default is 200, so reset will go above 100
  io.MousePos = ImVec2(x1, track_top + 50.0f);
  ScreenComponent::render(dl, p0, 220.0f, 1.0f, props);
  advance_frame();
  ...
  comp->params_[0].value =
      10000.0f; // high default is 4000, so reset will go below low_val
