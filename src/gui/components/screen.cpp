...
extern "C" {
EMSCRIPTEN_KEEPALIVE void load_ir_callback_screen(uintptr_t cab_ptr,
                                                  const char *path) {
  if (cab_ptr && path) {
...
void ScreenComponent::render(ImDrawList *dl, ImVec2 p0, float pedal_width,
...
void ScreenComponent::render_tuner_display(ImDrawList *dl, ImVec2 p0,
  ...
  if (tuner) {
    ...
    if (has_signal && note_idx >= 0) {
      ...
      dl->AddText(ImGui::GetFont(), ImGui::GetFontSize() * 2.0f,
                  ImVec2(note_x, display_y), Theme::TEXT_PRIMARY, note_buf);
      ...
      ImGui::SetCursorScreenPos(
          ImVec2(cx - cents_text_size.x * 0.5f, display_y));
      ...
      dl->AddRectFilled(ImVec2(bar_x, bar_y),
                        ImVec2(bar_x + bar_w, bar_y + bar_h), Theme::KNOB_BG,
                        3.0f * zoom);
      float center_x = bar_x + bar_w * 0.5f;
      dl->AddLine(ImVec2(center_x, bar_y - 1 * zoom),
                  ImVec2(center_x, bar_y + bar_h + 1 * zoom), Theme::TEXT_DIM,
                  1.5f * zoom);
      ...
      dl->AddText(ImGui::GetFont(), ImGui::GetFontSize() * 2.0f,
                  ImVec2(cx - ns_size.x * 1.5f, display_y), Theme::TEXT_DIM,
                  no_sig);
      display_y += 45 * zoom;
    ...
    ImGui::PushStyleColor(ImGuiCol_Text, mute_on
...
void ScreenComponent::render_ir_cabinet_display(ImDrawList *dl, ImVec2 p0,
                                                float pedal_width, float zoom,
                                                const ScreenProps &props) {
  ...
  if (ir_cab) {
      ...
      EM_ASM(
          {
            ...
            input.onchange = function(e) {
              ...
              reader.onload = function(re) {
                ...
                Module.ccall('load_ir_callback_screen', 'v',
                             [ 'number', 'string' ], [ cab_ptr, path ]);
              };
...
#else
      std::string path =
          show_open_dialog("Load Impulse Response", "WAV Audio", "wav");
      if (!path.empty()) {
        ir_cab->load_ir(path);
      }
#endif
    ...
    if (ir_cab->has_ir()) {
      ...
      ImGui::PushStyleColor(ImGuiCol_ButtonHovered,
                            ImVec4(0.35f, 0.15f, 0.12f, 1.0f));
      ImGui::PushStyleColor(ImGuiCol_ButtonActive,
                            ImVec4(0.50f, 0.20f, 0.15f, 1.0f));
      char clear_id[64];
...
void ScreenComponent::render_looper_display(ImDrawList *dl, ImVec2 p0,
                                            float pedal_width, float zoom,
                                            const ScreenProps &props) {
  ...
  if (has_loop && loop_len > 0) {
    progress =
        clamp(static_cast<float>(play_pos) / static_cast<float>(loop_len), 0.0f,
              1.0f);
  }
  ImGui::SetCursorScreenPos(ImVec2(p0.x + 15 * zoom, display_y));
  ...
  ImGui::PopStyleColor(3);
  if (ImGui::IsItemHovered())
    ImGui::SetTooltip("Start/stop recording (new loop)");

  ImGui::SameLine(0.0f, btn_gap);
  ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.14f, 0.30f, 0.18f, 1.0f));
  ImGui::PushStyleColor(ImGuiCol_ButtonHovered,
                        ImVec4(0.18f, 0.42f, 0.22f, 1.0f));
  ImGui::PushStyleColor(ImGuiCol_ButtonActive,
                        ImVec4(0.22f, 0.55f, 0.28f, 1.0f));
  char play_id[64];
  std::snprintf(play_id, sizeof(play_id), "Play/Stop##looper_play_%d",
                props.index);
  ...
  ImGui::PopStyleColor(3);
  if (ImGui::IsItemHovered())
    ImGui::SetTooltip("Toggle playback (keeps loop in memory)");
  ...
  ImGui::PushStyleColor(ImGuiCol_ButtonHovered,
                        ImVec4(0.40f, 0.34f, 0.12f, 1.0f));
  ImGui::PushStyleColor(ImGuiCol_ButtonActive,
                        ImVec4(0.52f, 0.45f, 0.15f, 1.0f));
  char dub_id[64];
  ...
  ImGui::PopStyleColor(3);
  if (ImGui::IsItemHovered())
    ImGui::SetTooltip("Clear loop from memory");
  ...
  if (!props.effect->params().empty()) {
    ...
    char slider_id[64];
    std::snprintf(slider_id, sizeof(slider_id), "##looper_level_%d",
                  props.index);
    ...
    if (ImGui::IsItemDeactivatedAfterEdit() &&
        s_popup_active_param_index == 0 && s_active_popup_id == popup_id) {
      if (level != s_popup_param_value_before_edit &&
...
void ScreenComponent::render_multiband_compressor_display(
    ImDrawList *dl, ImVec2 p0, float pedal_width, float zoom,
    const ScreenProps &props) {
  ...
  dl->AddLine(ImVec2(p0.x + 8.0f * zoom, p0.y + 48.0f * zoom),
              ImVec2(p1.x - 8.0f * zoom, p0.y + 48.0f * zoom),
              Theme::BORDER_DARK, 1.0f * zoom);
  ...
  auto render_mb_knob = [&](ImDrawList *dl, ImVec2 center, int pi, float radius,
    ...
    char label[64];
    std::snprintf(label, sizeof(label), "##knob_%s_%d_%d_%s",
                  props.effect->name(), props.index, pi, label_prefix);

    float r = radius * zoom;
    float knob_hit_size = r * Theme::KNOB_HIT_MULT;

    ImGui::SetCursorScreenPos(ImVec2(center.x - knob_hit_size * 0.5f,
    ...
    if (is_active && s_active_knob_id == label) {
      ...
      if (mdy != 0.0f) {
        ...
        float new_val =
            clamp(param.value + value_delta, param.min_val, param.max_val);
    ...
    if (s_knob_was_active && !is_active && s_active_param_index == pi &&
      ...
      if (new_val != s_param_value_before_drag &&
          props.on_commit_param_change) {
    ...
    if (is_hovered && std::fabs(ImGui::GetIO().MouseWheel) > 0.0f) {
      ...
      if (ImGui::GetIO().KeyShift)
        step *= 0.2f;
      float new_val = clamp(param.value + ImGui::GetIO().MouseWheel * step,
    ...
    if (ImGui::BeginPopup(label)) {
      ...
      if (ImGui::SliderFloat("##edit", &slider_val, param.min_val,
                             param.max_val, "%.2f")) {
      ...
      if (ImGui::IsItemDeactivatedAfterEdit() &&
          s_popup_active_param_index == pi && s_active_popup_id == label) {
        if (param.value != s_popup_param_value_before_edit &&
            props.on_commit_param_change) {
          props.on_commit_param_change(pi, s_popup_param_value_before_edit,
                                       param.value);
        }
        s_popup_active_param_index = -1;
        s_active_popup_id = "";
      ...
      if (props.gui_midi) {
        if (props.gui_midi->render_remove_mapping_item(props.effect->name(),
                                                       param.name)) {
          ImGui::CloseCurrentPopup();
        }
        if (props.gui_midi->render_learn_menu_item(props.effect->name(),
                                                   param.name)) {
          ImGui::CloseCurrentPopup();
        }
        ImGui::Spacing();
    ...
    for (int s = 0; s < segments; ++s) {
      ...
      ImU32 seg_color = filled ? ImGui::ColorConvertFloat4ToU32(led_color)
    ...
    ImU32 knob_bg = is_active
                        ? Theme::KNOB_ACTIVE
                        : (is_hovered ? Theme::KNOB_HOVER : Theme::KNOB_FACE);
    ...
    if (is_hovered || is_active) {
      std::string val_str =
          Theme::formatParameterValue(param.value, param.unit);
      std::string min_str =
          Theme::formatParameterValue(param.min_val, param.unit);
      std::string max_str =
          Theme::formatParameterValue(param.max_val, param.unit);
      std::string midi_info = props.gui_midi
                                  ? props.gui_midi->get_mapping_info(
                                        props.effect->name(), param.name)
                                  : "";

      if (param.tooltip.empty()) {
        ImGui::SetTooltip("%s: %s\nRange: [%s, %s]%s", param.name.c_str(),
    ...
    dl->AddText(
        ...
        is_active ? Theme::ACCENT_GOLD_HOT : Theme::TEXT_DIM,
        val_display.c_str());
  };

  // --- REUSABLE SLIDER HELPER (LAMBDA) ---
  auto render_xover_slider = [&](ImDrawList *dl, float track_x, int pi,
                                 const char *label_prefix, bool ticks_on_left) {
    (void)ticks_on_left;
    auto &param = params[pi];
    char label[64];
    std::snprintf(label, sizeof(label), "##slider_%s_%d_%d_%s",
                  props.effect->name(), props.index, pi, label_prefix);
    ...
    ImGui::InvisibleButton(label,
                           ImVec2(24.0f * zoom, track_bottom - track_top));
    ...
    if (is_active && s_active_knob_id == label) {
      ...
      if (pi == 0) {
        ...
        if (new_val >= high_val)
          new_val = high_val - 10.0f;
      } else if (pi == 1) {
        float low_val = params[0].value;
        if (new_val <= low_val)
          new_val = low_val + 10.0f;
      }
    ...
    dl->AddRectFilled(ImVec2(track_x - 1.5f * zoom, track_top),
                      ImVec2(track_x + 1.5f * zoom, track_bottom),
                      Theme::KNOB_TRACK_OFF, 1.5f * zoom);
    ...
    if (pi == 0) { // Low crossover (50 to 1000 Hz)
      ...
      float tick_hzs[] = {500.0f,  1000.0f,  4000.0f,
                          8000.0f, 12000.0f, 15000.0f};
      for (float hz : tick_hzs) {
        float norm = (hz - param.min_val) / range;
        float ty = track_bottom - norm * (track_bottom - track_top);
    ...
    ImU32 handle_bg = is_active
                          ? Theme::KNOB_ACTIVE
                          : (is_hovered ? Theme::KNOB_HOVER : Theme::KNOB_FACE);
    ImU32 border_col =
        (is_active || is_hovered) ? Theme::ACCENT_GOLD_HOT : Theme::ACCENT_GOLD;

    dl->AddRectFilled(ImVec2(track_x - 8.0f * zoom, handle_y - 5.0f * zoom),
                      ImVec2(track_x + 8.0f * zoom, handle_y + 5.0f * zoom),
                      Theme::KNOB_BG, 3.0f * zoom);
    dl->AddRectFilled(ImVec2(track_x - 7.0f * zoom, handle_y - 4.0f * zoom),
                      ImVec2(track_x + 7.0f * zoom, handle_y + 4.0f * zoom),
                      handle_bg, 2.0f * zoom);
    dl->AddRect(ImVec2(track_x - 8.0f * zoom, handle_y - 5.0f * zoom),
                ImVec2(track_x + 8.0f * zoom, handle_y + 5.0f * zoom),
                border_col, 3.0f * zoom, 0, 1.5f * zoom);
    ...
    dl->AddText(ImGui::GetFont(), ImGui::GetFontSize() * 0.75f,
                ...
                is_active ? Theme::ACCENT_GOLD_HOT : Theme::TEXT_SECONDARY,
                val_str.c_str());
  ...
  for (int b = 0; b < 3; ++b) {
    ...
    dl->AddRectFilled(ImVec2(meter_x, meter_y),
                      ImVec2(meter_x + meter_w, meter_y + meter_h),
                      Theme::METER_BG, 3.0f * zoom);
    dl->AddRect(ImVec2(meter_x, meter_y),
                ImVec2(meter_x + meter_w, meter_y + meter_h),
    ...
    if (norm_gr > 0.0f) {
      ...
      dl->AddRectFilled(
          ImVec2(fill_x0, meter_y + 1.0f * zoom),
    ...
    for (float db : tick_dbs) {
      ...
      dl->AddLine(ImVec2(tx, meter_y),
                  ImVec2(tx, meter_y + meter_h + 2.0f * zoom),
    ...
    render_mb_knob(dl, ImVec2(kx_left, p0.y + 120.0f * zoom), p_offset + 0,
                   k_radius, "thresh");
    render_mb_knob(dl, ImVec2(kx_right, p0.y + 120.0f * zoom), p_offset + 1,
                   k_radius, "ratio");

    // Row 2: Attack & Release
    render_mb_knob(dl, ImVec2(kx_left, p0.y + 185.0f * zoom), p_offset + 2,
                   k_radius, "attack");
    render_mb_knob(dl, ImVec2(kx_right, p0.y + 185.0f * zoom), p_offset + 3,
                   k_radius, "release");

    // Row 3: Makeup (Centered)
    render_mb_knob(dl, ImVec2(col_center, p0.y + 248.0f * zoom), p_offset + 4,
                   k_radius, "makeup");
  }
  ...
  render_mb_knob(dl,
                 ImVec2(p0.x + pedal_width - 40.0f * zoom,
                        p0.y + Theme::PEDAL_HEIGHT * zoom -
                            Theme::SWITCH_BOTTOM_OFFSET * zoom + 10.0f * zoom),
                 17, 13.0f, "outgain");
}
