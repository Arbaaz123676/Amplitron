...
void AudioEngine::process_audio(const float* input, float* output, int frame_count) {
    ...
    if (analyzer_on) {
        ...
        input_rms_.store(std::sqrt(sum_sq_in / std::max(frame_count, 1)),
    ...
    float peak_out = 0.0f;
    auto next_metronome_sample = [this]() -> float { return metronome_->next_sample(); };
    if (analyzer_on) {
        ...
        output_rms_.store(std::sqrt(sum_sq_out / std::max(frame_count, 1)),
                          std::memory_order_relaxed);
    ...
    if (recorder_->is_recording()) {
        recorder_->write_samples_stereo(process_buffer_.data(), process_buffer_right_.data(),
                                        frame_count);
    }
